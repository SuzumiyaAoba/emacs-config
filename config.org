...
起動時に GC が走らないように設定する。

#+begin_src emacs-lisp :tangle early-init.el
(defconst my/gc-cons-threshold-startup most-positive-fixnum)
(defconst my/gc-cons-threshold-normal (* 64 1024 1024))
...
** file-name-handler-alist を起動中だけ無効化
...
** Server (Emacs デーモン / emacsclient)

Emacs デーモン起動時も通常起動時も server を有効化する。
...
#+begin_src emacs-lisp :tangle init.el
;; Elpaca installer (from official README)
...
(fset 'setup--orig (symbol-function 'setup))

(defun my/elpaca-recipe-available-p (name)
  (condition-case nil
      (when-let* ((recipe (elpaca-recipe name))
                  (urlish (or (plist-get recipe :url)
...
(defmacro setup (name &rest body)
  "Wrap `setup' to auto-install packages with Elpaca."
  (declare (indent defun))
  (let ((elpaca-form nil)
        (clean-body nil))
    (dolist (form body)
      (if (and (consp form) (eq (car form) :elpaca) (null elpaca-form))
          (setq elpaca-form form)
        (push form clean-body)))
    (setq clean-body (nreverse clean-body))
    (if (and (symbolp name) (not (keywordp name)))
        (let* ((force-elpaca (and elpaca-form t))
               (recipe (cdr elpaca-form))
               (disable-elpaca (and elpaca-form (null (car recipe))))
               (order (cond
                       ((null elpaca-form) name)
                       ((or (eq (car recipe) t) (null (car recipe))) name)
                       ((symbolp (car recipe)) (if (cdr recipe)
                                                   (cons (car recipe) (cdr recipe))
                                                 (car recipe)))
                       (t (car recipe)))))
          `(if (and (not ,disable-elpaca)
                    (or ,force-elpaca (my/elpaca-recipe-available-p ',name)))
               (elpaca ,order (setup--orig ,name ,@clean-body))
             (setup--orig ,name ,@clean-body)))
      `(setup--orig ,name ,@clean-body))))
...
(defconst is-window window-system)

;; is-darwin-window
(defconst is-darwin-window (and is-darwin is-window))

;; when-darwin-not-window-system
(defconst is-darwin-not-window (and is-darwin is-window))
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
(setup simple
  (defun my/run-after-startup (fn &optional delay)
    "Run FN once after startup and DELAY seconds of idle time."
    (let ((delay (or delay 0.5)))
      (if after-init-time
        ...
        (add-hook 'emacs-startup-hook
                  (lambda ()
                    (run-with-idle-timer delay nil fn))))))
  (defun my/run-in-buffer (buffer fn)
    "Run FN in BUFFER if BUFFER is live."
    (when (buffer-live-p buffer)
      (with-current-buffer buffer
        (funcall fn))))
  (defun my/run-after-buffer-idle (fn &optional delay buffer)
    "Run FN in BUFFER after DELAY seconds of idle time."
    (run-with-idle-timer (or delay 0.1) nil
                         #'my/run-in-buffer
                         (or buffer (current-buffer))
                         fn))
  (defun my/require-feature-after-startup (feature &optional delay)
    "Require FEATURE after startup and DELAY seconds of idle time."
    (my/run-after-startup
     (lambda ()
...
(setup display-line-numbers
  ...
  (defun my/enable-display-line-numbers ()
    (unless (bound-and-true-p display-line-numbers-mode)
      (display-line-numbers-mode +1)))
  (defun my/enable-display-line-numbers-maybe ()
    (my/run-after-buffer-idle #'my/enable-display-line-numbers 0.1))
  (:with-mode prog-mode
    (:hook my/enable-display-line-numbers-maybe))
  (:with-mode text-mode
    (:hook my/enable-display-line-numbers-maybe)))
#+end_src
...
(setup catppuccin-theme
  (:opt catppuccin-flavor 'macchiato)
...
(setup keycast
  ...
  (keymap-global-set "C-c k" 'keycast-mode))
#+end_src
...
(setup hydra-posframe
  (:elpaca hydra-posframe :host github :repo "Ladicle/hydra-posframe")
  (:opt hydra-posframe-poshandler 'posframe-poshandler-window-bottom-center)
  (require 'hydra-posframe)
  (my/run-after-startup #'hydra-posframe-mode 1.5))
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
(setup projectile
  (:opt projectile-indexing-method 'alien
...
#+begin_src emacs-lisp :tangle init.el
(setup git-gutter
  (:opt git-gutter:update-interval 2)
  (:when-loaded
  ...
  (defun my/enable-git-gutter-in-buffer (buffer)
    (when (buffer-live-p buffer)
      (with-current-buffer buffer
        (unless (or (bound-and-true-p git-gutter-mode)
                    (null buffer-file-name)
                    (file-remote-p buffer-file-name))
          (git-gutter-mode +1)))))
  (defun my/enable-git-gutter-maybe ()
    (run-with-idle-timer 0.1 nil #'my/enable-git-gutter-in-buffer (current-buffer)))
  (add-hook 'find-file-hook #'my/enable-git-gutter-maybe))
#+end_src
...
(setup org-modern-indent
  (:only-if (and is-window (require 'org-modern-indent nil t)))
  (:with-mode org-mode
  ...
  (:opt org-roam-directory (file-truename "~/org-roam")
        ...
        '(("d" "default" plain "%?"
           :target (file+head "${my-id}.org"
                              "#+title: ${title}\n")
           :unnarrowed t)))
  (my/require-feature-after-startup 'org-roam 5.0))
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
(setup md-roam
  (:elpaca md-roam :host github :repo "nobiot/md-roam")
  (:load-after org-roam)
...
(setup treesit-auto
  ...
  (defun my/treesit-auto-apply-in-buffer ()
    (when (and buffer-file-name
               (not (file-remote-p buffer-file-name)))
      (when-let* ((fallback-mode major-mode)
                  (recipe (treesit-auto--get-mode-recipe fallback-mode))
                  (ts-mode (treesit-auto-recipe-ts-mode recipe))
                  ((not (eq fallback-mode ts-mode)))
                  ((fboundp ts-mode)))
        (let ((fallback-hook (intern (format "%s-hook" fallback-mode)))
              (treesit-language-source-alist (treesit-auto--build-treesit-source-alist)))
          (unless (treesit-auto--ready-p fallback-mode)
            (dolist (req-lang (ensure-list (treesit-auto-recipe-requires recipe)))
              (treesit-auto--prompt-to-install-package req-lang))
            (treesit-auto--prompt-to-install-package (treesit-auto-recipe-lang recipe)))
          (when (treesit-auto--ready-p fallback-mode)
            (funcall ts-mode)
            (when (boundp fallback-hook)
              (run-hooks fallback-hook)))))))
  (defun my/treesit-auto-apply-maybe ()
    (my/run-after-buffer-idle #'my/treesit-auto-apply-in-buffer 0.2))
  (add-hook 'find-file-hook #'my/treesit-auto-apply-maybe))
#+end_src
